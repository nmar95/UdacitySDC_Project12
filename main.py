...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # Upsample layer 4
    layer4_upsample = tf.layers.conv2d_transpose(layer7_conv, num_classes, 4, strides= (2, 2), padding= 'same',
                                    #kernel_initializer= tf.random_normal_initializer(stddev=0.01),
                                    kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
    ...
    # Upsample layer 3
    layer3_upsample = tf.layers.conv2d_transpose(layer4_skip, num_classes, 4, strides= (2, 2), padding= 'same',
                                    #kernel_initializer= tf.random_normal_initializer(stddev=0.01),
                                    kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
...
def run():
    ...
    # OPTIONAL: Train and Inference on the cityscapes dataset instead of the Kitti dataset. You'll need a GPU with at least 10 teraFLOPS to train on. https://www.cityscapes-dataset.com/
